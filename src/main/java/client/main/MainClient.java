...
public class MainClient {
	...
	public static void main(String[] args) {
		 ...
		 * a new one by accessing http://swe1.wst.univie.ac.at:18235/games with your web
		...
		NetworkConnector networkConnector = new NetworkConnector(gameId, serverBaseUrl);
		Map map = new Map();
...
//				.defaultHeader(HttpHeaders.ACCEPT, MediaType.APPLICATION_XML_VALUE).build();
		 ...
		 * example how to use GET requests is given below.
		 */
		/*
		 * PlayerRegistration playerReg = new PlayerRegistration("YourFirtName",
		 * "YourLastName", "YourStudentID");
		 * 
		 ...
		 * synchronous // one for the sake of simplicity. So calling block is fine.
		 * ResponseEnvelope<UniquePlayerIdentifier> resultReg = webAccess.block();
		 * 
		 * // always check for errors, and if some are reported at least print them to
		 * the // console (logging should be preferred) // so that you become aware of
		 * them during debugging! The provided server gives // you very helpful error
		 * messages. if (resultReg.getState() == ERequestState.Error) { // typically
		 * happens if you forgot to create a new game before the client // execution or
		 * // forgot to adapt the run configuration so that it supplies the id of the
		 * new // game to the client // open http://swe1.wst.univie.ac.at:18235/games in
		 * your browser to create a new // game and obtain its game id
